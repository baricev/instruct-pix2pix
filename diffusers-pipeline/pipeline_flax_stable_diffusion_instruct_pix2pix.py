...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        ```
"""
# NOTE: update __init__.py files at these three locations:
...
class FlaxStableDiffusionInstructPix2PixPipeline(FlaxDiffusionPipeline):
    r"""
    ...
    def prepare_image_latents(
        self, image, params, batch_size, num_images_per_prompt, do_classifier_free_guidance, key
    ):
        ...
        if (
            batch_size_adjusted > image_latents.shape[0]
            and batch_size_adjusted % image_latents.shape[0] == 0
        ):
    ...
    def _generate(
        ...
        # Create init_latents
        init_latent_dist = self.vae.apply(
            {"params": params["vae"]}, image, method=self.vae.encode
        ).latent_dist
